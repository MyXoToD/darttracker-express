...
export class AuthRepository extends BaseRepository<SessionEntity> {
  ...
  async findByUserIdAndRefreshToken(userId: number, refreshToken: string): Promise<SessionEntity | null> {
  ...
  async findByRefreshTokenAndRefreshTokenNotExpired(refreshToken: string): Promise<SessionEntity | null> {
    const [rows] = await db.query<SessionEntity[]>(
